...
with open("5_expedientes.csv", newline="", encoding="utf-8") as f_in, \
...
# === ETL Radicaciones (NUEVO) ===
print("\n📥 Procesando radicaciones...")
try:
    with open("4_1_radicaciones.csv", newline="", encoding="utf-8") as f_in, \
         open("etl_radicaciones.csv", "w", newline="", encoding="utf-8") as f_out:
        ...
        writer = csv.DictWriter(f_out, fieldnames=fieldnames_rad)
        writer.writeheader()
        
        count = 0
        for row in reader:
            expediente = row["Expediente"].strip()
            orden = int(row["Orden"]) if row["Orden"].strip() else None
            fecha_raw = row["Fecha"].strip()
            juzgado = row["Juzgado"].strip()
            fiscal_raw = row["Fiscal"].strip()
            fiscalia = row["Fiscalia"].strip()
            
            # Limpiar fiscal (remover prefijo "Fiscal:")
            fiscal_nombre = fiscal_raw.replace("Fiscal:", "").strip() if fiscal_raw else ""
            
            # Parsear fecha
            fecha_radicacion = parse_date(fecha_raw)
    ...
    print(f"✅ {count} radicaciones procesadas → etl_radicaciones.csv")

except FileNotFoundError:
    print("⚠️  Archivo 4_1_radicaciones.csv no encontrado, saltando procesamiento de radicaciones...")
except Exception as e:
    print(f"❌ Error procesando radicaciones: {e}")
    import traceback
    traceback.print_exc()
